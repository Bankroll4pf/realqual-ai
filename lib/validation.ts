...
export function validateLeadInput(payload: unknown): ValidationResult<LeadInput> {
  ...
  if (!isOneOf(preApprovalStatus, preApprovalStatuses)) errors.preApprovalStatus = "Select a valid pre-approval status.";
  if (!isOneOf(workingWithAgent, agentStatuses)) errors.workingWithAgent = "Select whether they are working with an agent.";
